globals [
  ...
  eroded-color    ;; color patches become when fully eroded
]
...
to setup
  ...
  ;; Set the target eroded color; replace with any color code or NetLogo color name
  set eroded-color orange - 3
...
to recolor  ;; patch procedure
  ifelse water = 0 or not show-water?
    ...
    [ degrade-color ]  ;; Change color based on water accumulation
end

to degrade-color
  ;; Change the patch color based on water accumulation toward eroded color
  let transition-factor (min list water 100) / 100  ;; Adjust for gradual transition
  set transition-factor min list transition-factor 1
  set pcolor scale-color eroded-color transition-factor 0 1
end
...
to go
  ...
  ;; Then do flow; shuffle the execution order of the patches each time
