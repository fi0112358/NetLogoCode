globals [
  ...
  erosion-center  ;; the patch where erosion starts
  erosion-radius  ;; radius around the erosion center where rainfall occurs
]

patches-own [
  original-color  ;; stores the original color of the patch
  elevation       ;; hidden elevation value, used for flow but not visualized
...
to setup
  ...
  set-patch-size 3
  import-pcolors "painting2.png"  ;; Import your image
  ...
  ask patches [
    ...
    set elevation random 125  ;; Initialize elevation randomly
    set water 0
    set drain? false
  ]

  ;; Make the edge patches drains
  ask patches with [count neighbors != 8] [
    set drain? true
    set elevation -100000000
  ]
  ...
  ;; Randomly select a starting patch for erosion
  set erosion-center one-of land
  set erosion-radius 1

  reset-ticks
end

to go
  ;; Perform rainfall within the erosion radius
  ask land with [distance erosion-center <= erosion-radius] [
    if random-float 1.0 < rainfall [
      set water water + 1  ;; Accumulate water when it rains
    ]
  ]

  ;; Perform flow on all land patches
  ask land [
    if water > 0 [ flow ]
  ]

  ;; Check and expand erosion radius
  expand-radius

  ;; Reset the drains to their initial state
  ask drains [
    set water 0
    set elevation -1000000
  ]
...
to expand-radius
  ;; Expand the erosion radius if any patch beyond the current radius has water
  if any? land with [water > 0 and distance erosion-center > erosion-radius] [
    set erosion-radius erosion-radius + 1
  ]
end

to flow  ;; patch procedure
  ;; Find the neighboring patch where the water level would be lowest if flowed there
  let target min-one-of neighbors [elevation + water]

  ;; The amount of flow is half the level difference, unless that much water isn't available
  let amount min list water (0.5 * (elevation + water - [elevation] of target - [water] of target))

  ;; Don't flow unless there's enough water
  if amount > 0 [
    set elevation elevation - (amount * 0.1)  ;; Simulate erosion by reducing elevation
...
to recolor  ;; patch procedure
  ifelse water = 0 or not show-water?
    [ set pcolor original-color ]  ;; Restore the original color if no water is present
    [ degrade-color ]  ;; Darken the color based on water accumulation
end

to degrade-color
  ;; Darken the patch color based on water accumulation
  let darkening-factor (min list water 100) / 200  ;; Adjust for gradual darkening
  set pcolor scale-color black darkening-factor 0 1
end
@#$#@#$#@
GRAPHICS-WINDOW
263
